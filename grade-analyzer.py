...
def get_avg_grade(grades, count_students): # O(1)
    if len(grades) == 0: # O(1)
        print("\nUnable to calculate the average grade — no student data available.")  # O(1)
        return # O(1)
    sum = 0 # O(1)
...
def get_highest_grade(grades, names, index=0, max_grade=0, max_indices=[]): # O(1)
    if len(grades) == 0: # O(1)
        print("Unable to determine the highest grade — no student data available.")  # O(1)
        return # O(1)
...
def count_passed(grades, index=0, count=0): # O(1)
    if len(grades) == 0: # O(1)
        print("No students found. Please enter student data.")  # O(1)
        return # O(1)
